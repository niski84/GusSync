...
func NewStateManager(stateFile string) (*StateManager, error) {
	...
	// CRITICAL: Clear all "completed" directory statuses that don't have discovered files tracking
	// This ensures backward compatibility - old "completed" directories get rescanned to find missed files
	sm.mu.Lock()
	...
	if clearedCount > 0 {
		fmt.Printf("Cleared %d old 'completed' directory statuses (will rescan to find missed files)\n", clearedCount)
